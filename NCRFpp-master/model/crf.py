...
def log_sum_exp(vec, m_size):
    ...
    """
    _, idx = torch.max(vec, 1)  # B * 1 * M, 1 means find the meximum in each row
    max_score = torch.gather(vec, 1, idx.view(-1, 1, m_size)).view(-1, 1, m_size)  # B * M
    '''
    torch.gather tutorial:
        the sencond parameter indicates dimension, 0 means row, 1 means columns
...
class CRF(nn.Module):
    ...
    def __init__(self, tagset_size, gpu):
        ...
        init_transitions[:,START_TAG] = -10000.0    # we can infer that each row represents transfering each tag to others
        init_transitions[STOP_TAG,:] = -10000.0     # row: STOP_TAG --->>> other tags
        init_transitions[:,0] = -10000.0            # why ?
