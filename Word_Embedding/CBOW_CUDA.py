...
class Model(object):
    ...
    def _buildModel(self):
        ...
        self._model = CBOW(len(self._vocab), self._embedding_dim, self._context_size).cuda()
    ...
    def _start(self):
        ...
        for epoch in range(self._epoch):
            ...
            for context, target in self._train_data:
                context_idxs = torch.tensor([self._word_to_ix[w] for w in context], dtype=torch.long).cuda()
                self._model.zero_grad()
                log_probs = self._model(context_idxs)
                loss = self._loss_function(log_probs, torch.tensor([self._word_to_ix[target]]).cuda())
                loss.backward()
